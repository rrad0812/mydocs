...
Debijan paketi, obično nazivani `debs`, su standardni format softverskih paketa u Ubuntuu. Mogu se identifikovati po `.deb` ekstenziji datoteke.
...
Odnos između paketa i bilo kojih drugih paketa prati Debijanovu politiku o binarnim zavisnostima, koju ćemo ovde ukratko pogledati. Najčešće na koje možete naići su: depends, recommends , i suggests (mada postoje i drugi!), pa ćemo pogledati ova tri.

-  : Apsolutno neophodno, paket neće raditi bez njega. Ako pokušamo da uklonimo
paket od kog zavisi drugi, oba će biti uklonjena!

-  : Jako zavisno, ali nije apsolutno neophodno (što znači da će paket bolje raditi
sa njim, ali može i bez njega)

-  : Nije potrebno, ali može na neki način poboljšati korisnost paketa.

Možemo videti, koristeći , tačno koji paketi spadaju u svaku od ovih kategorija. Hajde ponovo da upotrebimo Apache2 kao primer: apt show

```sh
apt show apache2


Ako pogledamo samo odeljke o zavisnostima, možemo videti da ssl-cert je to preporučeni paket:
...
U Ubuntuu, podrazumevana kon�guracija je podešena da instalira preporučene pakete zajedno sa , tako da kada smo pokrenuli komandu, je bio uključen u predložene pakete za instaliranje (iako je samo preporučen, a ne strogo neophodan). apt install depends apt
install apache2 ssl-cert
...
Tada izlaz postaje sledeći ( N ponovo otkucajte na promptu da biste za sada izbegli instaliranje):
...
Još jednom, apt nećemo nastaviti bez potvrde, pa dobijamo sledeći izlaz – pogledajmo pre nego što bilo šta izaberemo:
...
Ovo su paketi koji će biti direktno uklonjeni - rekli smo APT-u da želimo da uklonimo apache2-data , tako da očekujemo da će i to biti uključeno, ali će se i samo ukloniti apache2 ! To je zato što apache2-data je neophodna zavisnost i apache2 neće uopšte funkcionisati bez nje.

Hajde sada Y da potvrdimo da želimo da uklonimo ovu zavisnost.

Uklanjanje zavisnosti može, u najgorem slučaju, dovesti do toga da sistem postane neupotrebljiv – uvek treba biti oprezan kada to radite. Ako uklonite zavisnost koja je deo lanca, uklanjanja će se kaskadno širiti po lancu kako se svaka zavisnost i paket koji zavisi od nje budu uklanjali. Možete na kraju ukloniti više nego što ste prvobitno očekivali!

Dakle, uklonili smo pakete apache2 i apache2-data , ali ostale zavisnosti koje su instalirane pored njih apache2 su i dalje tu. Izlaz naše remove komande nam je dao naznaku kako da se nosimo sa ovim suvišnim paketima – `autoremove` komanda:
...
Možda se pitate zašto ne moramo da navodimo nikakve pakete kada pozivamo
autoremove komandu – na kraju krajeva, upravo smo se bavili paketima povezanim sa apache2 . To je zato što apt će proveriti sve pakete na vašem sistemu. Ispituje stablo zavisnosti i ako prvobitni razlog za instaliranje paketa više ne postoji (tj. nije ničemu potreban), biće označen za automatsko uklanjanje.

Ali!

Možda ćemo u budućnosti deinstalirati Apache2 bez deinstaliranja suvišnih paketa u tom trenutku. Možda smo pronašli drugu upotrebu za ssl-cert , možda u skripti koja koristi SSL serti�kate. Pa kako možemo zadržati ssl-cert paket, iako je označen za automatsko uklanjanje?
...
Ako ste radoznali, možete pokrenuti i da vidite kako se izlaz razlikuje za paket koji je jednom instaliran, a zatim uklonjen! apt list apache2
...
Generalno, podrazumevana kon�guracija paketa bi trebalo da radi dobro i da radi „odmah po pokretanju“ kada se instalira. Ali gotovo je neizbežno da ćemo, pre ili kasnije, želeti da prilagodimo paket kako bi bolje odgovarao našim potrebama.

Pre nego što pokušamo da prilagodimo paket, verovatno bi trebalo da pogledamo koje datoteke su uključene u njega. To možemo proveriti pomoću dpkg(1) , što je Debijanov menadžer paketa. Iako se APT sada češće koristi za osnovno rukovanje paketima, dpkg zadržava neke zaista korisne komande za ispitivanje datoteka i pronalaženje informacija o paketu. Podrazumevano je instaliran na Ubuntu sistemima tako da ga možemo direktno
koristiti:
...
Većina kon�guracije paketa se obrađuje putem kon�guracionih datoteka (često poznatih kao  ). Conf-faйlovi često sadrže stvari poput putanja do datoteka, logova i kon�guracije za otklanjanje grešaka, parametara jezgra (koji se mogu promeniti radi optimizacije performansi sistema), kontrole pristupa i drugih podešavanja kon�guracije.

Stvarni dostupni parametri će se razlikovati od jednog paketa do drugog.

Kon�guracione datoteke paketa se razlikuju od svih ostalih datoteka koje se isporučuju u paketu. Paket može imati bilo koji broj kon�guracionih datoteka (uključujući i nijednu!).

Kon�guracione datoteke eksplicitno označava održavalac paketa tokom razvoja kako bi zaštitio lokalnu kon�guraciju od prepisivanja tokom nadogradnji i kako bi se vaše izmene sačuvale. To nije slučaj ni sa jednim drugim tipovima datoteka – izmene koje napravite na redovnim datotekama u tom paketu biće prepisane tokom nadogradnje.

Pošto mi možemo da menjamo kon�ikatnu datoteku, možemo doći do sukoba kada
održavalac paketa promeni te iste datoteke. Stoga je važno razumeti kako se takvi sukobi rešavaju.

Možemo prikazati četiri moguća scenarija nadogradnje koristeći sledeću tabelu. Šta se dešava tokom nadogradnje zavisi od toga da li smo mi promenili kon�guracioni fajl na našem sistemu („promenio/nije promenio korisnik“) i da li je podrazumevani sadržaj verzije promenio održavalac paketa („promenio/nije promenio održavalac“):

Zadrži korisničke izmene Pitajte korisnika

Nema promena koje treba Primeni promene iz
napraviti ažuriranja

Dakle, možemo videti da ako napravimo izmene u kon�guracionoj datoteci, APT nikada neće prepisati naše izmene bez prethodnog pitanja.

Iz liste datoteka u paketu, kako znamo koje su kon� datoteke?

Na kraju krajeva, nisu označeni nikakvom posebnom ekstenzijom datoteke, i iako se često nalaze u /etc/ direktorijumu, ne moraju biti tamo. Kao što smo ranije videli, jedina stvar koju conf-faйlovi imaju zajedničko jeste da je održavalac paketa odlučio da ih označi kao takve.

Ali to je naš trag! Dakle, još jednom, dpkg može nam priskočiti u pomoć. Sledeća komanda će nam prikazati ( --show ) podskup datoteka u paketu koje je održavalac apache2 označio kao „ Conffiles “ ( ) i prikazuje svaku u novom redu ( ) na izlazu: -f='${Conffiles}\n' \n

```sh
dpkg-query --show -f='${Conffiles}\n' apache2
```

Ako želite da saznate više o tome šta ova komanda radi, možete pogledati stranicu uputstva tako što ćete otkucati , a ona će vas provesti kroz sve opcije. man dpkg-query --show

Za razliku od , nam takođe daje niz slova i brojeva. Ovaj niz je poznat kao ili . dpkg --listfiles dpkg-query
...
Kontrolna suma je poput otiska prsta - jedinstvena je za svaku verziju datoteke, tako da će svaki put kada se datoteka promeni dobiti novu kontrolnu sumu - što nam omogućava da vidimo  u odnosu na podrazumevanu vrednost.

Hajde da postavimo situaciju kako bismo mogli malo da isprobamo ovu ideju. Možemo početi tako što ćemo napraviti neke izmene u kon� fajlu. U Apache2, glavni kon� fajl je /etc/apache2/apache2.conf , pa hajde da to iskoristimo. U situaciji kada podešavamo novi veb server, razumno je da želimo da povećamo LogLevel sa „warn“ na „debug“ da bismo dobili više poruka o debagovanju, pa hajde da pokrenemo ovu komandu i koristimo sed da bismo napravili tu promenu u kon� fajlu:
...
Zatim ćemo ponovo pokrenuti naš Apache2 server kako bismo mogli da aktiviramo naše izmene kon�guracije:
...
A zatim ponovo pokrenite veri�kaciju sa:
...
Možda ste primetili da postoji „c“ pored gornjeg reda, ali ne i donjeg – „c“ pokazuje da je datoteka con�le.
...
Korišćenjem --reinstall zastavice, možemo prisiliti apt ponovno raspakivanje celog podrazumevanog sadržaja. Ako zatim ponovo proverimo…
...
Možemo koristiti za nadogradnju instaliranog paketa, ali ovo će nadograditi samo na najnoviju verziju. U našem slučaju, već smo bili na najnovijoj verziji Apache2, pa smo morali da nateramo APT da ponovo raspakuje sadržaj kako bi prepisao naše „glupe“ izmene. sudo apt install <package>
...
Ovo sumnjivo liči na listu kon�guracionih datoteka koju smo ranije videli, zar ne?

Kako se ispostavilo, uklanjanje paketa ne uklanja automatski i kon� datoteke. Ali – ovo je namerno, radi naše udobnosti.

Ostavljanjem kon�guracionih datoteka na mestu, ako odlučimo da ponovo instaliramo apache2 u budućnosti, nećemo morati da trošimo vreme na ponovno podešavanje cele naše kon�guracije.
...
Primetite da nas ovaj put nije pitalo da potvrdimo da li želimo da nastavimo. Zašto ne? Kao što smo ranije videli, potvrda „D/n“ se prikazuje kada postoje zavisnosti, a mi znamo da Apache2 ima zavisnosti.
...
Pošto su zavisnosti i kon�guracione datoteke još uvek tu, možemo odmah koristiti našu prethodnu kon�guraciju. Ona čak zadržava i izmene koje smo ranije napravili, što možemo proveriti ponovnim pogledom na kontrolnu sumu:

```sh
md5sum /etc/apache2/apache2.conf
```

Šta ako odlučimo da ne želimo izmenjene kon�guracione datoteke? Možda želimo da se vratimo na podrazumevanu instalaciju ili znamo da više nikada nećemo želeti da koristimo paket – kako možemo osigurati da se sve kon�guracione datoteke uklone istovremeno kada uklonimo paket?
...
Ovaj mali indikator nam govori da će paket biti uklonjen I očišćen. Međutim, on i dalje ne uklanja zavisnosti (ili kon�guracione datoteke tih zavisnosti).

Hajde da Y ponovo otkucamo da bismo potvrdili da želimo da nastavimo. Zatim, kada se uklanjanje završi, možemo još jednom proveriti listu:
...
Takođe bismo mogli da koristimo komandu od ranije i nećemo pronaći pakete koji odgovaraju . dpkg-query --
show -f='${Conffiles}\n' apache2 dpkg-query apache2
...
Kao što smo ranije videli, možemo pretraživati bazu podataka APT paketa po ključnim rečima koristeći da bismo pronašli softver koji bismo možda želeli da instaliramo. Takođe možemo videti sve pakete koje već imamo koristeći , mada može biti lakše za navigaciju i informativnije ako umesto toga koristimo – tada možemo koristiti tastere sa strelicama gore i dole na tastaturi za skrolovanje (ili pritisnuti da bismo se vratili na terminalnu liniju). apt search
<keyword> apt list dpkg -l Q
...
Znamo da Apache2 trenutno nije instaliran, jer smo ga uklonili i obrisali, zbog čega se instalirana verzija prikazuje kao „nijedan“:
...
Ispod svake verzije prikazuje se i  . Najnovija verzija ( 2.4.58-1ubuntu8.4 ) dolazi sa noble-updates (main) i noble-security (main). Originalna verzija ( 2.4.58-1ubuntu8 ) dolazi sa noble (main). Ovo nam govori da je ovo bila verzija objavljena sa 24.04 LTS (Noble Numbat).
...
The following packages have unmet dependencies:
 apache2 : Depends: apache2-bin (= 2.4.58-1ubuntu8) but 2.4.58-1ubuntu8.4 is to be installe
...
Možda se sada pitate „odakle tačno dolaze svi ovi paketi?“. U ovom tutorijalu smo ukratko pomenuli nekoliko izvora, ali im još nismo posvetili direktnu pažnju. Hajde da sada odvojimo malo vremena da de�nišemo šta podrazumevamo pod svim ovim različitim izvorima iz kojih
APT može da preuzima pakete.

APT može da preuzima pakete.

Izvor iza APT-a je  . Ova arhiva je podeljena na mnogo slojeva, svaki sa
...
Hajde da ukratko pogledamo ovaj dijagram. Opšti tok je da se Arhiva deli na  . Svaka serija je podeljena na  , a zatim svaki džep sadrži četiri  . Ako bismo pokušali da sve ovo prikažemo na jednom dijagramu, bilo bi prilično opširno, pa hajde da pogledamo jednu putanju.

Serija je skup paketa koji se objavljuju sa određenom verzijom Ubuntua – obično se nazivaju po svom kodnom imenu (npr. i na našem dijagramu)  Svaka verzija Ubuntua može imati više izdanja (na primer, LTS će imati početno izdanje kada se pokrene (npr. 24.04 LTS), a zatim „naknadna izdanja“ (npr. 24.04.1 LTS) – sva su to dela iste serije (
). mantic noble oracular noble

U praksi, ljudi često koriste termine „Ubuntu izdanje“ i „Ubuntu serija“ naizmenično.

Svaka Ubuntu serija ( noble , jammy , itd.) je podeljena na  , koji su povezani sa životnim ciklusom razvoja/objavljivanja softvera:

•  sadrži pakete kakvi su u vreme objavljivanja.

•  sadrži ažuriranja paketa dok se testiraju.

• Kada se ažuriranje objavi, ono dolazi ili iz  ili  , u zavisnosti
od toga da li je u pitanju ažuriranje vezano za bezbednost ili ne.

• I  , koji sadrži pakete koji nisu bili dostupni u vreme objavljivanja.
...
DŽepovi se obično dodaju na kraj serije, i prilično je uobičajeno videti crticu ( - ) kada se govori o džepovima.
...
Svaki džep je podeljen na četiri  , u zavisnosti od toga da li su paketi koje sadrže otvorenog ili zatvorenog koda , i da li ih zvanično podržava Canonical ili ih održava Ubuntu zajednica:

glavni ograničeno

univerzum multiverzum

•  sadrži pakete otvorenog koda koje zvanično podržava Canonical. Ovi
paketi su ili instalirani na svakoj Ubuntu mašini ili se veoma široko koriste za različite tipove sistema i slučajeva upotrebe.

•  sadrži sve ostale pakete otvorenog koda u Ubuntuu, koje obično održavaju Debijan i Ubuntu zajednice, ali može da uključuje i dodatnu bezbednosnu pokrivenost od strane Kanonikala u okviru Ubuntu Pro , koji je dostupan besplatno za ličnu upotrebu na do pet mašina.

•  sadrži pakete koje zvanično podržava Canonical, ali nisu dostupni pod potpuno besplatnom licencom.

•  sadrži vlasnički softver koji održava zajednica – ove pakete kompanija Canonical uopšte ne podržava.
...
Ako ikada budemo želeli da instaliramo .deb datoteku, APT je de�nitivno najpogodniji način da to uradimo. Možda ćemo i dalje morati ručno da obrađujemo neke zavisnosti, ali sada imamo znanje da to možemo da uradimo.

Srećom, većinu paketa koji će vam ikada zatrebati verovatno ćete pronaći preko APT-a. Ako nije, vredi proveriti da li je softver dostupan kao „  (brza verzija).
...
Umesto  kao što ih imamo u debovima, snapovi koriste koncept kanala da bi  de�nisali koja verzija snapa je instalirana.

Podrazumevano, snimci se automatski ažuriraju, tako da ne moramo da pamtimo da ih ažuriramo i nadograđujemo. Postoje trenuci na živom sistemu, kao što je server u proizvodnom okruženju, kada možda ne želimo da se ažuriranja automatski primenjuju. U tim slučajevima, možemo isključiti automatska ažuriranja i osvežiti sistemske snimke kada nam je
to zgodno (na primer, tokom perioda održavanja).
...
Čestitamo, stigli smo do kraja! Obradili smo dosta materijala u ovom tutorijalu, pa hajde da Čestitamo, stigli smo do kraja! Obradili smo dosta materijala u ovom tutorijalu, pa hajde da ukratko rezimiramo šta smo naučili:

- Kako ažurirati i nadograditi sav softver našeg sistema pomoću APT-a: * sudo apt update && sudo apt upgrade

- Kako pretraživati softver pomoću ključnih reči ili nizova znakova: * ili apt search <keyword> apt search "some content string"

- Kako videti opis paketa, uključujući i zavisnosti koje on ima: * apt show <package name>

Ili kako da proverite koje su verzije paketa dostupne: * apt policy <package>

- Kako instalirati pakete… * sudo apt install <package1> <package2>

- Kako videti sve datoteke koje paket sadrži * dpkg --listfiles <package>

- Kako saznati kom paketu pripada datoteka: * dpkg --search <path/to/file>

- ...I kako ponovo ukloniti pakete! Kao i razlika između uklanjanja i čišćenja. * sudo apt
remove <package>

- Čak smo naučili kako da vratimo starije verzije APT paketa i sve o APT izvorima.

- Kako pronaći kon� datoteke u paketu: * dpkg-query --show -f='${Conffiles}\n'
<package>

- Kako videti da li su datoteke paketa promenjene: * dpkg --verify <package>

- ...A ako je ne-kon�ikova datoteka slučajno promenjena, možemo je popraviti sa: * sudo
apt install --reinstall <package>

- Znamo da su naše izmene u conf datotekama uvek bezbedno sačuvane, dok se izmene u datotekama koje nisu conf datoteke vraćaju na prethodno stanje pri sledećoj nadogradnji ili bezbednosnoj ispravci.

- Važno je napomenuti da znamo kako da proverimo kontrolne sume pomoću md5sum ili sličnih alata, što nam pomaže da bezbednije gradimo pakete iz izvornog koda.
